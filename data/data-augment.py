...
def create_our_cci_col(df):
    new_col = 6 * df["metastatic_romano_tm1"] + 1 * df["chf_romano_tm1"] + 1 * df["dementia_romano_tm1"] + 2 * df["renal_elixhauser_tm1"] + 1 * df["wtloss_elixhauser_tm1"] + 2 * df["hemiplegia_romano_tm1"] + df["alcohol_elixhauser_tm1"] + df["tumor_romano_tm1"] + df["arrhythmia_elixhauser_tm1"] + df["pulmonarydz_romano_tm1"] + df["coagulopathy_elixhauser_tm1"] + 2 * df["compdiabetes_elixhauser_tm1"] + df["anemia_elixhauser_tm1"] + df["electrolytes_elixhauser_tm1"] + df["liver_elixhauser_tm1"] + df["pvd_elixhauser_tm1"] + df["psychosis_elixhauser_tm1"] + df["pulmcirc_elixhauser_tm1"] + 6 * df["hivaids_romano_tm1"] + df["hypertension_elixhauser_tm1"]
    #new_col = df["bps_mean_t"]
    return new_col
