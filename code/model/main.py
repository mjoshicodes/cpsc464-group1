...
def load_data_df():
    ...
    # data_fp = os.path.join(git_dir, 'data', 'data_dropped_cost_with_cci.csv')
    # data_fp = os.path.join(git_dir, 'data', 'data_with_cci.csv')
...
def get_Y_x_df(df, verbose):
    ...
    # labels (Y) to predict
    Y_predictors = ['log_cost_t', 'gagne_sum_t', 'log_cost_avoidable_t', 'our_gagne_score', 'our_cci_score']
...
def main():
    ...
    # train model with Y = 'our_cci_score'
    our_cci_score_r2_df, \
    pred_our_cci_score_df, \
    our_cci_score_lasso_coef_df = model.train_lasso(train_df,
                                                         holdout_df,
                                                         x_column_names,
                                                         y_col='our_cci_score',
                                                         outcomes=Y_predictors,
    ...
    holdout_our_gagne_score_df = get_split_predictions(pred_our_gagne_score_df,
                                                          split='holdout')
    holdout_our_cci_score_df = get_split_predictions(pred_our_cci_score_df,
                                                          split='holdout')

    holdout_pred_df = pd.concat([holdout_df, holdout_log_cost_df,
                                 holdout_gagne_sum_t_df,
                                 holdout_log_cost_avoidable_df,
                                 holdout_our_gagne_score_df,
                                 holdout_our_cci_score_df], axis=1)
