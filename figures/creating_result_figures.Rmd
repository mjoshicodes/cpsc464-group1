...
table_2_filename <- 'table2_cci_added.csv'
...
table2_no_cost_boxplot <-ggplot(df_table2_no_cost_plot, aes(x = predictor, y = `Race black`, fill=predictor)) +
  geom_bar(position=position_dodge(), stat="identity",
           colour='black') +
  geom_errorbar(aes(ymin=`Race black` - `Race black SE`, ymax=`Race black` + `Race black SE`), width=.2)
table2_no_cost_boxplot_title <- table2_boxplot + labs(title="Fraction of Black Patients in the group with the highest risk by predictor")
```
...
table2_no_cost_boxplot <- ggplot(df_table2_no_cost_plot, aes(x = predictor, y = `Race black`, fill=predictor)) +
  geom_bar(position=position_dodge(), stat="identity",
           colour='black') +
  geom_errorbar(aes(ymin=`Race black` - `Race black SE`, ymax=`Race black` + `Race black SE`), width=.2)
table2_no_cost_boxplot_title <- table2_boxplot + labs(title="Fraction of Black Patients (DROPPED COSTS)")
```
